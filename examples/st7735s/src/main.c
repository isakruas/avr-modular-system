...
static inline void loop()
{
    ...
    ST7735S_FillScreenWithColor(0x000);

    // Example string to draw
    char fullAsciiString[] = "!\"#$ %&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~";
    uint16_t pos1 = ST7735S_DrawString(0, 0, fullAsciiString, 0xFFFF, 0x0000);
    uint16_t pos2 = ST7735S_DrawString(pos1 & 0xFF, (pos1 >> 8) & 0xFF, fullAsciiString, 0xFFFF, 0x0000);
    ST7735S_DrawString(pos2 & 0xFF, (pos2 >> 8) & 0xFF, fullAsciiString, 0xFFFF, 0x0000);
    _delay_ms(1000);
    ...
    convertIntToString(counter, counterString); // Call the function to convert the counter to a string

    // Draw the "Hello, world!" string
    ST7735S_DrawString(0, 0, helloWorldString, 0xFFFF, 0x0000);

    // Draw the counter string
    ST7735S_DrawString(6 * 6, 8 * 8, counterString, 0xFFFF, 0x0000);
    ...
    ST7735S_FillScreenWithColor(0xFFFF); // White color
    _delay_ms(1000);

    ST7735S_FillScreenWithColor(0xF800); // Red color
    _delay_ms(1000);

    ST7735S_FillScreenWithColor(0x07E0); // Green color
    _delay_ms(1000);

    ST7735S_FillScreenWithColor(0x001F); // Blue color
    _delay_ms(1000);

    ST7735S_FillScreenWithColor(0xFFE0); // Yellow color
    _delay_ms(1000);
